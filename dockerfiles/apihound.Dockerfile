...
ENV VERSION_PKG="github.com/specterops/apihound/cmd/api/src/version"
...
RUN --mount=type=cache,target=/go/pkg/mod go build -C cmd/api/src -o /apihound -ldflags "$(cat LDFLAGS)" github.com/specterops/apihound/cmd/api/src/cmd/bhapi

########
# Package APIHound
################
FROM gcr.io/distroless/static-debian11 AS apihound
ARG SHARPHOUND_VERSION
ARG AZUREHOUND_VERSION

COPY --from=api-builder /apihound /opt/apihound /etc/apihound /var/log /
COPY dockerfiles/configs/apihound.config.json /apihound.config.json
...
COPY --from=hound-builder /tmp/sharphound/sharphound-${SHARPHOUND_VERSION}.zip /etc/apihound/collectors/sharphound/
COPY --from=hound-builder /tmp/sharphound/sharphound-${SHARPHOUND_VERSION}.zip.sha256 /etc/apihound/collectors/sharphound/
COPY --from=hound-builder /tmp/azurehound/artifacts/azurehound-${AZUREHOUND_VERSION}.zip /etc/apihound/collectors/azurehound/
COPY --from=hound-builder /tmp/azurehound/artifacts/azurehound-${AZUREHOUND_VERSION}.zip.sha256 /etc/apihound/collectors/azurehound/

ENTRYPOINT ["/apihound", "-configfile", "/apihound.config.json"]
