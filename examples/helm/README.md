# APIHound Community Edition Helm Chart Example
...
```
$ helm install apihound $BH_ROOT/examples/helm/.
```

## Accessing
By default, the ingress is enabled. This means the application will be available on the ingress endpoint with the host set to `apihound.example.com` (Each of these values can be configured in the values.yaml). As long as you have a properly configured ingress controller and valid DNS configuration the application will be available at `https://apihound.example.com`, else without DNS you can test with curl by passing the 'Host' Header: `curl -H 'Host: apihound.example.com' https://$endpointIP`.

 If you have a TLS cert you can enable `apihound.tls.customCert` in the values.yaml and provide the ingress secret in `apihound.tls.certSecret`. Else, the provided cert will be self signed from the ingress controller.

 ## Configuring APIHound Community Edition

To configure the Helm Chart deployment of APIHound Community Edition you can use the two files specified below:

-   `values.yaml` - A general Helm Chart configuration file - you can use this to configure aspects of the deployment of APIHound and set Environment Variables. This file generally will be the single source of truth for application and deployment configuration.
-   `templates/cmbh.yaml` - This is the Kubernetes configmap used for the APIHound Application - you can add your own custom configuration in the `apihound-config.json` section of this file. Some of it is populated with the `values.yaml` as well for portibility
...
-   8080 - APIHound Web Port. This is the backend port of the service used by APIHound. 
...
By Default the `latest` tag is set. You can change this by setting the `apihound.container.version` in the values.yaml
...
Validate all 3 of the Deployments are healthy, Services are active and the ingress is properly configured  with `kubectl get -n apihoundad deployments,svc,ingress`

Assure the cluster DNS is resolving the dns for the 'app-db' service and the 'graph-db' service. An easy way to validate this is to spin up a busybox pod in the apihoundad namespace and use nslookup to check for resolution issues.

```
$ kubectl run -it --rm dns-test -n apihoundad --image busybox:latest
