...
The config file `build.config.json.template` is designed to have sane defaults for most APIHound configuration. Copy
it to a new file `build.config.json` and target it for building the app locally.

The config file `integration.config.json.template` is designed to have sane defaults for most APIHound integration testing configuration.
